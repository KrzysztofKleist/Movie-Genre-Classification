...
from movieframe_dataset_slcr import MovieFrameDataset
...
train_set_all = MovieFrameDataset(
    ...
    "files_slcr/{}.tsv".format(train_list),
    transform=transform,
    raw=True,
)
test_set_all = MovieFrameDataset(
    "{}/".format(test_frames_type),
    "files_slcr/{}.tsv".format(test_list),
    transform=transform,
    raw=True,
)
...
create_dir("results_slcr")
create_dir(
    "results_slcr/{}_{}_{}_{}_epochs".format(
        train_frames_type, train_list, model.__class__.__name__, str(NUM_EPOCHS)
    )
)

# File path
train_losses_path = "results_slcr/{}_{}_{}_{}_epochs/{}_losses.txt".format(
...
# File path
test_losses_path = "results_slcr/{}_{}_{}_{}_epochs/{}_losses.txt".format(
...
plt.legend()
plt.savefig(
    "results_slcr/{}_{}_{}_{}_epochs/{}_{}_{}_{}_losses.png".format(
        train_frames_type,
        train_list,
...
####################################################################################
# File path
label_preds_path = "results_slcr/{}_{}_{}_{}_epochs/{}_labels.txt".format(
    train_frames_type, train_list, model.__class__.__name__, str(NUM_EPOCHS), train_list
)
# Open the file in write mode (this will create the file if it doesn't exist)
...
# File path
label_test_path = "results_slcr/{}_{}_{}_{}_epochs/{}_labels.txt".format(
    train_frames_type, train_list, model.__class__.__name__, str(NUM_EPOCHS), test_list
)
# Open the file in write mode (this will create the file if it doesn't exist)
...
genres = ["Drama", "Act_Adv", "Com_Rom", "Thr_Hor_Cri"]
...
classification_report_path = (
    "results_slcr/{}_{}_{}_{}_epochs/classification_report.txt".format(
        train_frames_type, train_list, model.__class__.__name__, str(NUM_EPOCHS)
    )
)
# Open the file in write mode (this will create the file if it doesn't exist)
with open(classification_report_path, "w") as file:
    file.write(
        "batch_size: {}, lr: {}, num_epochs: {}, step_size: {}, gamma: {}\naccuracy: {}%".format(
            BATCH_SIZE, LR, NUM_EPOCHS, STEP_SIZE, GAMMA, accuracy
        )
        + "\n"
    )
    file.write(
    ...
    file.write("{:12} {:>12} {:>12} {:>12} {:>12}".format("Genre", *genres) + "\n")

    for i in range(len(genres)):
        file.write("{:12} {:12} {:12} {:12} {:12}".format(genres[i], *cm[i]) + "\n")
