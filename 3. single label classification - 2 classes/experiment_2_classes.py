...
from movieframe_dataset_2_classes import MovieFrameDataset
...
train_set_all = MovieFrameDataset(
    ...
    "files_2_classes/{}.tsv".format(train_list),
    transform=transform,
    raw=True,
)
test_set_all = MovieFrameDataset(
    "{}/".format(test_frames_type),
    "files_2_classes/{}.tsv".format(test_list),
    transform=transform,
    raw=True,
)

train_set = torch.utils.data.Subset(
    train_set_all, list(range(0, int(0.2 * len(train_set_all))))
)
test_set = torch.utils.data.Subset(
    test_set_all, list(range(0, int(0.2 * len(test_set_all))))
)
...
create_dir("results_2_classes")
create_dir(
    "results_2_classes/{}_{}_{}_{}_epochs".format(
        train_frames_type, train_list, model.__class__.__name__, str(NUM_EPOCHS)
    )
)

# File path
train_losses_path = "results_2_classes/{}_{}_{}_{}_epochs/{}_losses.txt".format(
...
# File path
test_losses_path = "results_2_classes/{}_{}_{}_{}_epochs/{}_losses.txt".format(
...
plt.legend()
plt.savefig(
    "results_2_classes/{}_{}_{}_{}_epochs/{}_{}_{}_{}_losses.png".format(
        train_frames_type,
        train_list,
...
####################################################################################
# File path
label_preds_path = "results_2_classes/{}_{}_{}_{}_epochs/{}_labels.txt".format(
    train_frames_type, train_list, model.__class__.__name__, str(NUM_EPOCHS), train_list
)
# Open the file in write mode (this will create the file if it doesn't exist)
...
# File path
label_test_path = "results_2_classes/{}_{}_{}_{}_epochs/{}_labels.txt".format(
    train_frames_type, train_list, model.__class__.__name__, str(NUM_EPOCHS), test_list
)
# Open the file in write mode (this will create the file if it doesn't exist)
...
for i in range(len(label_test)):
    corrects_list.append(int(label_test[i] == label_preds[i]))

accuracy = round(sum(corrects_list) / len(label_test), 4) * 100

genres = ["Act_Adv", "Thr_Hor_Cri"]

label_test_np = np.array([np.array(x) for x in label_test])
label_preds_np = np.array([np.array(x) for x in label_preds])
cm = confusion_matrix(label_test_np.argmax(axis=1), label_preds_np.argmax(axis=1))
...
classification_report_path = (
    "results_2_classes/{}_{}_{}_{}_epochs/classification_report.txt".format(
        train_frames_type, train_list, model.__class__.__name__, str(NUM_EPOCHS)
    )
)
# Open the file in write mode (this will create the file if it doesn't exist)
with open(classification_report_path, "w") as file:
    file.write(
        "batch_size: {}, lr: {}, num_epochs: {}, step_size: {}, gamma: {}\naccuracy: {}%\nepoch_time: {} seconds".format(
            BATCH_SIZE, LR, NUM_EPOCHS, STEP_SIZE, GAMMA, accuracy, convert_time(training_time/NUM_EPOCHS)
        )
        + "\n"
    )
    file.write(
    ...
    file.write("{:12} {:>12} {:>12}".format("Genre", *genres) + "\n")

    for i in range(len(genres)):
        file.write("{:12} {:12} {:12}".format(genres[i], *cm[i]) + "\n")

print("accuracy:")
print(str(accuracy) + "%")
print()

print("label_test length:")
print(len(label_test))
print("label_preds length:")
